...
namespace MoreCitizenUnits
{
    ...
    internal static class UnitUtils
    {
        ...
        private static void EnsureCitizenUnits(ushort buildingID, ref Building data, int homeCount, int workCount, int visitCount, int studentCount, int hotelCount)
        {
            ...
            if (citizenManager.CreateUnits(out uint firstUnit, ref Singleton<SimulationManager>.instance.m_randomizer, buildingID, 0, homeCount, workCount, visitCount, 0, studentCount, hotelCount))
            {
